...
function formDecorator($delegate) {
  ...
  function MessagesFormController($element,
    ...
    if (this.$name) {
      ...
      console.debug('Instantiating patched controller for form %s',
        this.$name);
      ...
      _.extend(this, {
         ...
         * form proper, then tell $messages to broadcast an event that
         * the form's validity changed (somewhat).
         * @this FormController
         */
        $setValidity: function $messagesSetValidity(token,
          value,
          control) {
          ...
          if (formHasControl &&
            ...
            $messages.$broadcast(this, control);
