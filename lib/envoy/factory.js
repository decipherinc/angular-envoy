...
function envoyFactory($http, $q) {
  ...
  prototype = {
    ...
    emit: function emit(form, control) {
      ...
      function eventData(ctrls) {
        return $q.all(_(ctrls)
          ...
          .then(function (pileOfMessages) {
            ...
            var messages = {};
            var maxErrorLevel = _.reduce(pileOfMessages,
              function (result, formMessages, idx) {
                var form = ctrls[idx].$form;
                var errorLevelName = $envoy._formErrorLevel(form,
                  formMessages);
                var errorLevel = LEVELS[errorLevelName];
      ...
      _emit = _.debounce(function _emit(form) {
        var boundCtrls = _.toArray(bindings[form.$name]);
        var messageCtrls = _.filter(boundCtrls, function (ctrl) {
          return ctrl.$form;
        });
        var childCtrls;
        var parentCtrls;

        childCtrls = _(messageCtrls)
          .map(function (ctrl) {
        ...
        try {
          $envoy._emitting =
            $q.all([
              //eventData(messageCtrls),
              eventData(childCtrls),
              eventData(messageCtrls.concat(parentCtrls))
            ])
              .then(function (data) {
                //var ctrlData = data[0];
                var childData = data[0];
                var parentData = data[1];

                _.each(childCtrls.concat(messageCtrls).concat(parentCtrls),
                  function (ctrl) {
                    ctrl.$viewData.reset();
                  });
                _.each(childCtrls, function (ctrl) {
                  ctrl.update(childData);
                });
                _.each(parentCtrls.concat(boundCtrls), function (ctrl) {
                  ctrl.update(parentData);
                });
                //_.each(boundCtrls, function (ctrl) {
                //  ctrl.update(ctrlData);
                //});
                debug('Emitted from controls: %s', boundCtrls.join(', '));
              })
              .catch(function (err) {
                debug('Error during emit attempt:', err);
              });
        } catch (err) {
          debug('Error during emit attempt:', err);
        }
